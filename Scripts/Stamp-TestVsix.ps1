...
.DESCRIPTION
    ...
    "vs-marketplace-test" (publisher "jessehouwing") that is currently published, increments its
    last segment by one, copies the fixture, rewrites the <Identity Version="..."> in its embedded
    extension.vsixmanifest to that new value, and writes the result to -OutputPath so every test
    run publishes the next version in sequence.
...
.PARAMETER ExtensionId
    Marketplace internal extension name to look up. Defaults to "vs-marketplace-test", matching
    tests/sample-extension/publish-manifest.json's identity.internalName.
...
Write-Host "Stamped $OutputPath with version $Version"
