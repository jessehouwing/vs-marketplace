...
describe('VsixPublisher', () => {
  ...
  describe('login', () => {
    ...
    it('should throw error when vswhere fails', async () => {
      ...
      await expect(publisher.login('test-publisher', 'test-token')).rejects.toThrow(
        'Could not locate VsixPublisher.exe'
      );
    });
  ...
  describe('logout', () => {
    it('should logout successfully after login', async () => {
      ...
      expect(execOutputCalls[execOutputCallsBefore].command).toBe('C:\\VS\\VsixPublisher.exe');
  ...
  describe('publish', () => {
    ...
    it('should publish successfully with valid inputs', async () => {
      ...
      await publisher.publish('C:\\path\\to\\extension.vsix', 'C:\\path\\to\\manifest.json');

      const execCalls = adapter.getExecCalls();
      expect(execCalls).toHaveLength(execCallsBefore + 1);
      expect(execCalls[execCallsBefore].command).toBe('C:\\VS\\VsixPublisher.exe');
    ...
    it('should throw error when publish fails', async () => {
      ...
      await expect(
        publisher.publish('C:\\path\\to\\extension.vsix', 'C:\\path\\to\\manifest.json')
