...
describe('GitHubAdapter', () => {
  ...
  describe('exec', () => {
    ...
    it('should still fail on stderr when both ignoreReturnCode and failOnStdErr are true', async () => {
      await expect(
        adapter.exec(process.execPath, ['-e', "console.error('stderr-output'); process.exit(7)"], {
          ignoreReturnCode: true,
          failOnStdErr: true,
          silent: true,
        })
  ...
  describe('fileExists', () => {
    ...
    it('should return false when file does not exist', async () => {
      const path = await import('path');
      const os = await import('os');
      const result = adapter.fileExists(path.join(os.tmpdir(), 'nonexistent-file-gh-12345.txt'));
