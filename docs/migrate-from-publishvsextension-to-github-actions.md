...
extension) to the `jessehouwing/vs-marketplace@v0.9.0` GitHub Actions action.

## Migration approach

1. Move your pipeline trigger to a GitHub Actions workflow `on:` block.
2. Replace the `PublishVisualStudioExtension@5` step with `jessehouwing/vs-marketplace@v0.9.0`.
...
2. Add an `azure/login@v2` step before `jessehouwing/vs-marketplace@v0.9.0`.
...
### After (`jessehouwing/vs-marketplace@v0.9.0` with PAT)
  ...
  publish:
    ...
    steps:
      ...
      - name: Publish to VS Marketplace
        uses: jessehouwing/vs-marketplace@v0.9.0
        with:
          auth-type: pat
...
### After (`jessehouwing/vs-marketplace@v0.9.0` with OIDC)
